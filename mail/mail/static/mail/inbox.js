...
function load_mailbox(mailbox) {
  ...
  if (mailbox === "inbox" || mailbox === "sent" || mailbox === "archive") {
      loadEmails(mailbox); // Example: Load inbox emails
  }
...
function loadEmails(mailbox) {
  ...
  fetch(url)
    ...
    .then(data => {
        const parentElement = document.getElementById('emails-view');
        ...
        data.forEach(email => {
          ...
          p.textContent = `Subject: ${email.subject}, Body: ${email.body}`;
...
function sendEmail(recipients, subject, body) {
  ...
  .then(data => {
      ...
      console.log(data);
      console.log("Running loademails")
      loadEmails("sent");
  });
}